...
namespace QuickSVG
{
    ...
    public class QuickSVGWriter
    {
        ...
        private static RectD rectMax = new RectD(isValid: false);
        ...
        public QuickSVGWriter(FillRule fillrule = FillRule.EvenOdd,
            ...
            foreach (PolyInfo pi in PolyInfoList)
            {
                ...
                foreach (PathD path in pi.paths)
                {
                    ...
                    for (int j = 1; j < path.Count; j++)
                    {
                        ...
                        (path[j][0] * scale + offsetX),
                        (path[j][1] * scale + offsetY)));
                    }
...
#else
                            writer.Write("<text x=\"{0:f2}\" y=\"{1:f2}\">{2},{3}</text>\n", (pt[0] * scale + offsetX), (pt[1] * scale + offsetY), pt[0], pt[1]);
#endif
                        }
                    }
                    writer.Write("</g>\n\n");
